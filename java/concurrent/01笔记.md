...
![完全并行执行操作示意图.png](img/完全并行执行操作示意图.png)
```java
// 订单队列
Vector<P> pos;
// 派送单队列
...
- Queue
  ...
  - 单端阻塞队列：
    ...
    - DelayQueue 支持延时出队。
![单端阻塞队列.png](img/单端阻塞队列.png)

  - 双端阻塞队列：LinkedBlockingDeque
![双端阻塞队列.png](img/双端阻塞队列.png)
...
解决ABA问题的思路：增加一个版本号维度
每次执行 CAS 操作，附加再更新一个版本号，只要保证版本号是递增的，那么即便 A 变成 B 之后再变回 A，版本号也不会变回来（版本号递增的）

---
原子类概览: 原子化的基本数据类型、原子化的对象引用类型、原子化数组、原子化对象属性更新器和原子化的累加器
![原子类组成概览图.png](img/原子类组成概览图.png)

1. 原子类的基本数据类型：AtomicBoolean、AtomicInteger 和 AtomicLong...
2. 原子化的对象引用类型: AtomicReference、AtomicStampedReference 和 AtomicMarkableReference
3. 原子化数组：AtomicIntegerArray、AtomicLongArray 和 AtomicReferenceArray...
4. 原子化对象属性更新器: AtomicIntegerFieldUpdater、AtomicLongFieldUpdater 和 AtomicReferenceFieldUpdater ， 注意：对象属性必须是 volatile 类型的，只有这样才能保证可见性
 ...
 * keepAliveTime & unit：表示空闲线程最大存活时间。如果一个线程空闲了keepAliveTime & unit这么久，而且线程池的线程数大于 corePoolSize ，那么这个空闲的线程就要被回收了。
 * workQueue：工作队列
...
不建议使用 Executors 的最重要的原因： Executors 提供的很多方法默认使用的都是无界的 LinkedBlockingQueue，高负载情境下，无界队列很容易导致 OOM，而 OOM 会导致所有请求都无法处理，这是致命问题。

Executors使用建议
1. 使用有界队列
2. 慎用默认拒绝策略：线程池默认的拒绝策略会 throw RejectedExecutionException 这是个运行时异常，对于运行时异常编译器并不强制 catch 它，所以开发人员很容易忽略
3. 对线程的执行要做好安全方案，捕获异常并按需处理：通过 ThreadPoolExecutor 对象的 execute() 方法提交任务时，如果任务在执行的过程中出现运行时异常，会导致执行任务的线程终止；不过，最致命的是任务虽然异常了，但是你却获取不到任何通知，这会让你误以为任务都执行得很正常
...
---
CompletableFuture
1. runAsync(Runnable runnable)和supplyAsync(Supplier<U> supplier)，它们之间的区别是：Runnable 接口的 run() 方法没有返回值，而 Supplier 接口的 get
() 方法是有返回值的。
```java
// 使用默认线程池
...
// 异步保存所有报价  
for (int i=0; i<3; i++) {
  Integer r = bq.take(); // Retrieves and removes the head of this queue, waiting if necessary until an element becomes available.
...
![简版分治任务模型图.png](img/简版分治任务模型图.png)
